...
pub(crate) fn map_type(fstr: &str, ty: &syn::Type) -> proc_macro2::TokenStream {
    match ty {
        syn::Type::Path(tp) => map_path(fstr, tp),
        syn::Type::Array(ta) => {
            let inner = map_type(fstr, &ta.elem);
...
fn map_segs(fstr: &str, seg: &syn::PathSegment) -> proc_macro2::TokenStream {
            ...
            syn::PathArguments::AngleBracketed(angle_args) => {
                ...
                match args.len() {
                        1 => match args.first().unwrap() {
                            syn::GenericArgument::Type(t) => map_type(fstr, t),
...
fn map_path(fstr: &str, tp: &syn::TypePath) -> proc_macro2::TokenStream {
    if tp.path.segments.is_empty() {
        ...
        map_segs(fstr, tp.path.segments.first().unwrap())
    } else {
        let ids = tp
            .path
            .segments
